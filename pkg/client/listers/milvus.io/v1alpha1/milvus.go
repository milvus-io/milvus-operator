...
type MilvusLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Milvus, err error)
...
// List lists all Milvuses in the indexer.
func (s *milvusLister) List(selector labels.Selector) (ret []*v1alpha1.Milvus, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Milvus))
	})
	return ret, err
}
...
type MilvusNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.Milvus, error)
...
// List lists all Milvuses in the indexer for a given namespace.
func (s milvusNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Milvus, err error) {
...
// Get retrieves the Milvus from the indexer for a given namespace and name.
func (s milvusNamespaceLister) Get(name string) (*v1alpha1.Milvus, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("milvus"), name)
	}
	return obj.(*v1alpha1.Milvus), nil
}
